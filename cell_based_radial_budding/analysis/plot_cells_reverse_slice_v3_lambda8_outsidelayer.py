...
# import data
f=open('/global/cscratch1/sd/cschreck/calc_vel_budding_damped_annulus/prod_reverse_radial_layer8.0_desync0.4_b4e3_att0.0_seed-1_axial.dat','r')
...
for line in f.readlines():
    ...
    if len(linesplit)==1:
        ...
        x[frame-1][ncell-1]=(float(linesplit[0])+float(linesplit[1]))/math.sqrt(2.0)
        y[frame-1][ncell-1]=-(float(linesplit[0])-float(linesplit[1]))/math.sqrt(2.0)
...
#binsize=3.2
binsize=2.8
frontbins=int(2.*np.pi*radius[frameplot+1]/binsize)
binanglesize=2.*np.pi/np.float(frontbins)
xfront=np.zeros((nframes,frontbins+1))+1e16
yfront=np.zeros((nframes,frontbins+1))+1e16
xlayer=np.zeros((nframes,frontbins+1))
ylayer=np.zeros((nframes,frontbins+1))
for frame in range(0,nframes):
    # calc frontline
    for i in range(0,n[frame]): 
        if(depth[frame][i]<2.):
...
#plt.plot(xlayer[frameplot,:],ylayer[frameplot,:],'-k',linewidth=2)
plt.plot([-60.,0.,60.],[100.,0.,100.],'w',linewidth=6)
...
plt.axis('off')
plt.tight_layout()
plt.savefig('system_reverse_slice_v3_lambda8_lightoutside.pdf')
plt.savefig('system_reverse_slice_v3_lambda8_lightoutside.png')
plt.show() 
